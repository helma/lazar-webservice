= LAZAR-WEBSERVICE

This is an example webservice for the <tt>lazar</tt>[lazar.in-silico.de] prediction system and public data from the DSSTOX[http://www.epa.gov/nheerl/dsstox/] project.

== REST API

Description							Method	URI																	Parameters

List all endpoints			GET 		/endpoints
List all categories			GET 		/categories
Endpoint definition			GET 		/{category}/{database}/{endpoint}	
Predict smiles					GET 		/{category}/{database}/{endpoint}		smiles
Create a model 					POST 		/{category}/{database}/{endpoint}		structure_file, activity_file 		
Update a model 					PUT 		/{category}/{database}/{endpoint}		structure_file, activity_file

== Installation

to be completed
